...
import { Users, Package, FileText, Search, ChevronRight, LogOut } from 'lucide-react';
...
const AdminDashboard = ({ setAuth }) => {
    ...
    const handleLogout = async () => {
        try {
            const res = await adminAxios.post('/logout');
            if (res.data.success) {
                setAuth(false); // Reset global auth state
            ...
            <div className="card">
                ...
                <div className="space-y-2">
                    {filtered.length > 0 ? (
                        filtered.map(user => (
                            <div key={user._id} onClick={() => navigate(`/user/${user._id}`)} className="p-4 hover:bg-slate-50 rounded-xl flex justify-between items-center cursor-pointer transition-colors border border-transparent hover:border-slate-100 group">
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 bg-slate-900 text-white rounded-xl flex items-center justify-center font-black shadow-lg shadow-slate-900/20">
                                        {user.name[0]}
                                    </div>
                                    <div>
                                ...
                                <div className="flex items-center gap-3">
                                    <span className={`px-2 py-1 rounded text-[10px] font-black uppercase ${user.isPremium ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-500'}`}>
                                        {user.planType}
                                    </span>
                        ...
                        <p className="text-center py-10 text-slate-400 font-medium">No merchants found matching your search.</p>
                    )}
                </div>
            </div>
        </div>
...
const StatCard = ({icon: Icon, label, value, color}) => (
    <div className="card flex items-center gap-4 border-l-4 border-l-slate-900">
        <div className={`p-4 ${color} text-white rounded-2xl shadow-lg shadow-current/30`}>
            <Icon size={24}/>
        </div>
        <div>
